...
the list: the stamp moves and the row goes. *Show in Explorer* reveals it. *Recycle Bin* sends
it there, one at a time, and History records it. A folder handed over from another tab is added
to the list and walked at once, and the sender hears how many were never opened.

## What it refuses to do

It does not walk on opening, and it does not delete in bulk: a file never opened is a file that
was wanted once, and the point is to look at each, not to clear a number. It does not read
