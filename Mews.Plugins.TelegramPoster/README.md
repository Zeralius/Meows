...
A Mews plugin for [telegram-posting-bot](https://github.com/Zeralius/telegram-posting-bot).
Browse its groups and queues, see what goes out next, edit group settings, and run it.

The bot is public, so the setup panel prefills its URL. You can clone it from inside Mews
...
Plugin id `mews.telegram-poster`. Needs **Python**, and **git** for the clone step.
...
1. **Clone.** Repository URL and destination, defaulting to a sibling of the Mews checkout.
...
directory. With Mews and the bot as sibling checkouts, that just works.
...
Interval groups count from the bot's own start time, which Mews cannot know, so the left column
...
crashing, so hand-editing `config.json` outside Mews is covered too.
...
`%APPDATA%\Mews\plugins\mews.telegram-poster\settings.json` holds the bot folder path, the
