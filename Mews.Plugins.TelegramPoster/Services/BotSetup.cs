...
public static class BotSetup
{
    ...
    /// someone with Mews but not the link can still get a working checkout.
    /// </summary>
    ...
    /// <summary>Next to the Mews checkout if we can find one, since that is the usual layout.</summary>
    public static string DefaultCloneDestination()
    {
        var dir = new DirectoryInfo(AppContext.BaseDirectory);
        while (dir is not null)
        {
            if (File.Exists(Path.Combine(dir.FullName, "Mews.sln")) && dir.Parent is not null)
