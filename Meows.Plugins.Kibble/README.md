...
Plugin id `meows.kibble`. It reads the same `telegram-posting-bot` checkout that
[Telegram Poster](../Meows.Plugins.TelegramPoster/README.md) uses, and the two share
`Meows.Bot.Core/` so their idea of the bot's rules cannot drift apart.
...
places near the Meows checkout. A folder counts only if it holds `bot.py` and `config.json`.
