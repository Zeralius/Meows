...
public sealed class PluginCatalog
{
    ...
    ///
    /// MEOWS_PLUGINS_DIR is a <c>;</c> separated list and adds to the rest rather than
    ...
    public static IReadOnlyList<string> ResolvePluginsDirectories()
    {
        ...
        var fromEnv = Environment.GetEnvironmentVariable("MEOWS_PLUGINS_DIR");
    ...
    private IEnumerable<PluginDescriptor> LoadFrom(string assemblyPath)
    {
        ...
        {
            if (!typeof(IMeowsPlugin).IsAssignableFrom(type) || type.IsAbstract || type.IsInterface)
                continue;
            if (type.GetConstructor(Type.EmptyTypes) is null)
            {
                _log.Write("plugins", $"{type.FullName} implements IMeowsPlugin but has no parameterless constructor.");
                continue;
            }
